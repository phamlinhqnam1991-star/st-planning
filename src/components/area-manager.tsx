...
export function AreaManager(){
 ...
 async function load(){const r=await fetch("/api/area",{cache:"no-store"});const d=await safeJson(r);if(!r.ok){setStatus(`Lỗi: ${apiError(d)}`);return}setAreas(d.areas);setMaps(d.mappings);setGroups(d.groups)}
 useEffect(()=>{load()},[]);
 ...
 async function add(){setStatus("");const r=await fetch("/api/area",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(form)});const d=await safeJson(r);if(!r.ok){setStatus(`Lỗi: ${apiError(d)}`);return}setForm({area_code:"",area_name:"",description:"",sort_order:"0"});setStatus("Đã thêm Area.");await load()}
 async function toggle(a:Area){const r=await fetch("/api/area",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({id:a.id,is_active:!a.is_active})});const d=await safeJson(r);if(!r.ok)setStatus(`Lỗi: ${apiError(d)}`);else await load()}
