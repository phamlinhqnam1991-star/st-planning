...
export function AreaManager(){
 ...
 async function load(){const r=await fetch("/api/area",{cache:"no-store"});const d=await r.json();if(!r.ok){setStatus(`Lỗi: ${apiError(d)}`);return}setAreas(d.areas);setMaps(d.mappings);setGroups(d.groups)}
 useEffect(()=>{load()},[]);
 ...
 async function add(){setStatus("");const r=await fetch("/api/area",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(form)});const d=await r.json();if(!r.ok){setStatus(`Lỗi: ${apiError(d)}`);return}setForm({area_code:"",area_name:"",description:"",sort_order:"0"});setStatus("Đã thêm Area.");await load()}
 async function toggle(a:Area){const r=await fetch("/api/area",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({id:a.id,is_active:!a.is_active})});const d=await r.json();if(!r.ok)setStatus(`Lỗi: ${apiError(d)}`);else await load()}
 async function edit(a:Area){const name=prompt("Area Name",a.area_name);if(name===null)return;const code=prompt("Area Code",a.area_code);if(code===null)return;const description=prompt("Description",a.description||"");if(description===null)return;const order=prompt("Sort Order",String(a.sort_order));if(order===null)return;const r=await fetch("/api/area",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({id:a.id,area_name:name,area_code:code,description,sort_order:Number(order)})});const d=await r.json();if(!r.ok)setStatus(`Lỗi: ${apiError(d)}`);else{setStatus("Đã cập nhật Area.");await load()}}
 async function saveGroups(){if(!selected)return;const checked=[...document.querySelectorAll<HTMLInputElement>('input[name="stgroup"]:checked')].map(x=>x.value);const r=await fetch("/api/area/groups",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({area_id:selected,groups:checked})});const d=await r.json();if(!r.ok)setStatus(`Lỗi: ${apiError(d)}`);else{setStatus("Đã lưu ST Group → Area.");await load()}}
 return <>
 <div className="card"><h2 style={{marginTop:0}}>+ Add Area</h2><div className="area-form"><input className="input" placeholder="Area Code" value={form.area_code} onChange={e=>setForm({...form,area_code:e.target.value})}/><input className="input" placeholder="Area Name" value={form.area_name} onChange={e=>setForm({...form,area_name:e.target.value})}/><input className="input" placeholder="Description" value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/><input className="input" type="number" placeholder="Sort Order" value={form.sort_order} onChange={e=>setForm({...form,sort_order:e.target.value})}/><button className="btn primary" onClick={add}>Add Area</button></div></div>
 <div className="card section" style={{overflowX:"auto"}}><h2 style={{marginTop:0}}>Area List</h2><table><thead><tr><th>Code</th><th>Area Name</th><th>Description</th><th>ST Groups</th><th>Status</th><th>Action</th></tr></thead><tbody>{areas.map(a=><tr key={a.id}><td>{a.area_code}</td><td><b>{a.area_name}</b></td><td>{a.description||""}</td><td>{maps.filter(m=>m.area_id===a.id).map(m=>m.st_group).join(", ")||"—"}</td><td><span className="badge">{a.is_active?"Active":"Inactive"}</span></td><td><div className="row"><button className="btn" onClick={()=>{setSelected(a.id);setStatus("")}}>Assign Groups</button><button className="btn" onClick={()=>edit(a)}>Edit</button><button className={a.is_active?"btn danger-btn":"btn"} onClick={()=>toggle(a)}>{a.is_active?"Deactivate":"Activate"}</button></div></td></tr>)}</tbody></table></div>
 {selected&&<div className="card section"><h2 style={{marginTop:0}}>Assign ST Groups → {areas.find(a=>a.id===selected)?.area_name}</h2><p className="muted">Một ST Group thuộc một Area. Chọn group đã thuộc Area khác sẽ tự chuyển sang Area này.</p><div className="group-grid">{groups.map(g=><label className="check-card" key={`${selected}-${g}`}><input name="stgroup" type="checkbox" value={g} defaultChecked={selectedGroups.includes(g)}/><span><b>{g}</b>{assigned.get(g)&&assigned.get(g)!==selected?<small>Hiện tại: {areas.find(a=>a.id===assigned.get(g))?.area_name}</small>:null}</span></label>)}</div><div className="row" style={{marginTop:16}}><button className="btn primary" onClick={saveGroups}>Save Group Assignment</button><button className="btn" onClick={()=>setSelected(null)}>Close</button></div></div>}
 </>}
