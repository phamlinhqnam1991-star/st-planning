...
export function StGroupManager({rows}:{rows:G[]}){
 ...
 async function save(){if(!f.st_group.trim())return alert("Nhập ST Group.");setBusy(true);try{
  const r=await fetch("/api/master/st-group",{method:edit?"PATCH":"POST",headers:{"content-type":"application/json"},body:JSON.stringify(f)});const d=await safeJson(r);if(!r.ok)throw new Error(d.error);location.reload()
 ...
 async function deactivate(g:G){if(!confirm(`Deactivate ST Group ${g.st_group}?`))return;setBusy(true);try{
  const r=await fetch("/api/master/st-group",{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({st_group:g.st_group})});const d=await safeJson(r);if(!r.ok)throw new Error(d.error);location.reload()
 }catch(e){alert(e instanceof Error?e.message:String(e))}finally{setBusy(false)}}
