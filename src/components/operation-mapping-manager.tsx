...
export function OperationMappingManager({rows,groups,sourceOperations}:{rows:Row[];groups:string[];sourceOperations:string[]}){
 ...
 return <div>
  <div className="card section"><h2 style={{marginTop:0}}>{edit?"Sửa Operation Mapping":"+ Thêm Operation Code vào nhóm"}</h2>
  ...
  <div className="card section"><div className="row" style={{marginBottom:14}}><input className="input" placeholder="Tìm mã công đoạn / nhóm..." value={q} onChange={e=>setQ(e.target.value)}/><span className="muted">{filtered.length} mapping</span></div>
