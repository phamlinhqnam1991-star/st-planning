...
export function OperationMappingManager({rows,groups,sourceOperations}:{rows:Row[];groups:string[];sourceOperations:string[]}){
 ...
 async function save(){
  ...
  setBusy(true);try{
   ...
   const r=await fetch("/api/master/operation-mapping",{method,headers:{"content-type":"application/json"},body:JSON.stringify(body)});const d=await r.json();if(!r.ok)throw new Error(d.error||"Save failed");location.reload();
 ...
 return <div>
  <div className="card section"><h2 style={{marginTop:0}}>{edit?"Edit Operation Mapping":"+ Add Operation Code to Group"}</h2><p className="muted">Operation Code bị bỏ khỏi nhóm sẽ chuyển inactive, không xóa lịch sử. Khi Save/Remove, ST Routing Mapping được refresh tự động.</p>
   ...
   </div><div className="row" style={{marginTop:12}}><button className="btn primary" disabled={busy} onClick={save}>{busy?"Đang lưu...":edit?"Save Changes":"Add Mapping"}</button>{edit&&<button className="btn" onClick={cancel}>Cancel</button>}</div>
  </div>
  <div className="card section"><div className="row" style={{marginBottom:14}}><input className="input" placeholder="Tìm Operation Code / ST Group..." value={q} onChange={e=>setQ(e.target.value)}/><span className="muted">{filtered.length} mappings</span></div>
   <div style={{overflowX:"auto"}}><table><thead><tr><th>ST Group</th><th>Operation Code</th><th>Source Label</th><th>Standard Operation</th><th>Rule</th><th>Action</th></tr></thead><tbody>{filtered.map(r=><tr key={r.id}><td><b>{r.st_group}</b></td><td>{r.source_operation_code}</td><td>{r.source_label||""}</td><td>{r.standard_operation_rule}</td><td><span className="badge">{r.mapping_rule}</span></td><td><div className="row"><button className="btn small" disabled={busy} onClick={()=>startEdit(r)}>Edit / Move</button><button className="btn danger-btn small" disabled={busy} onClick={()=>remove(r)}>Remove</button></div></td></tr>)}</tbody></table></div>
  </div>
 </div>
}