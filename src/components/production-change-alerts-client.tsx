...
const statusLabel=(x:ProductionChangeAlert)=>x.nextMainStatus==="ATTENTION_NEW"?"Đang chờ Main sau":x.nextMainStatus==="WAITING_BATCH"?"Chưa có lô Main sau":x.nextMainStatus==="ACKNOWLEDGED"?"Main sau đã nhận":x.nextMainStatus==="NO_NEXT_MAIN"?"Main cuối":"Đang theo dõi";
...
export function ProductionChangeAlertsClient({items}:{items:ProductionChangeAlert[]}){
 ...
 const direct=items.filter(x=>x.sourceKind==="DIRECT").length;const pending=items.filter(x=>x.nextMainStatus==="ATTENTION_NEW"||x.nextMainStatus==="WAITING_BATCH").length;
 return <div className="production-alert-workspace">
  <div className="production-alert-summary">
   <div><b>{items.length}</b><span>thay đổi từ Production</span></div><div><b>{direct}</b><span>Job thêm ngoài lô</span></div><div><b>{pending}</b><span>cần Main sau chú ý</span></div>
  </div>
  ...
  {rows.length===0?<div className="notice">Không có thay đổi Production phù hợp bộ lọc.</div>:<div className="production-alert-list">{rows.map(x=><article key={x.id} className={`production-alert-card impact-${(x.impactLevel||"INFO").toLowerCase()}`}>
   <header><div><span className={`production-alert-kind ${x.sourceKind.toLowerCase()}`}>{x.sourceKind==="DIRECT"?"PRODUCTION ADD":"FROM PREVIOUS MAIN"}</span><b>Job {x.jobNum}</b><span>được thêm vào <b>{x.batchNo}</b> · {x.standardOperation}</span></div><span className="production-alert-time">{dt(x.approvedAt||x.createdAt)}</span></header>
