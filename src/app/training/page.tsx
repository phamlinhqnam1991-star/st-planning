// V491 Training sync: Scheduling workload keeps READY/WAIT handoff focus, flat green READY cards, compact columns, and WAIT breakdown with Job+pcs+dm² including Chemical Line.
...
export default async function Page({searchParams}:{searchParams:Promise<{job?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page training-page">
   <div className="erp-page-head guide-head"><div><div className="erp-object-eyebrow">ONBOARDING · LIVE DATABASE · ST PLANNING · V491</div><h2>New User Training — học bằng dữ liệu thật đang chạy</h2><p>Lý thuyết trước, sau đó đối chiếu ngay Operation Code, Main, Area, Planner, Recipe, Batch Rules, Process Time và một Job thật từ database. V491 dùng cùng classifier READY/WAIT với Planning Board và tối giản Scheduling Workload: READY/WAIT/HOLD là trọng tâm, READY dùng card xanh phẳng dễ đọc, Flybar recipe-only và WAIT Next Main breakdown có Job + pcs + dm² ở mọi khu vực kể cả Chemical Line.</p></div><div className="erp-command-actions"><Link className="btn" href="/logic-guide">Logic & Hướng dẫn</Link><Link className="btn primary" href="/job-tracker">Mở Job Tracker</Link></div></div>
   ...
   <section id="flow" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>7 · Trainer dẫn học viên đi xuyên flow — từng câu hỏi phải trả lời</b></div></div><div className="lg-body"><ol className="lg-steps">
    ...
    <li><b>Planning Board:</b> Job READY, WAIT · Next Main hay WAIT · Future Mains? Tạo Batch ở Main đúng, không tạo ở Scheduling. WAIT · Next Main là blocker gần nhất sau frontier; WAIT · Future Mains là các Main xa hơn. Nếu cùng Job/Main nhiều batch thì Planning Board hiển thị nối bằng &.</li>
