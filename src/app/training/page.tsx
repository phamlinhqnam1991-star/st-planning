...
export default async function Page({searchParams}:{searchParams:Promise<{job?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page training-page">
   <div className="erp-page-head guide-head"><div><div className="erp-object-eyebrow">ONBOARDING · LIVE DATABASE · ST PLANNING · V527</div><h2>New User Training — học bằng dữ liệu thật đang chạy</h2><p>Lý thuyết trước, sau đó đối chiếu ngay Operation Code, Main, Area, Planner, Recipe, Batch Rules, Process Time, ST Output dm²/ngày và một Job thật từ database. V527 giữ cách xác định candidate theo V523; riêng ST Final Steps lọc thêm NextOperation theo active ST Scope và chỉ nhận ST Only, Main Planning hoặc Intermediate. Các Alert/Audit và Batch/Schedule engine cũ vẫn giữ nguyên.</p></div><div className="erp-command-actions"><Link className="btn" href="/logic-guide">Logic & Hướng dẫn</Link><Link className="btn" href="/st-output">ST Output</Link><Link className="btn primary" href="/job-tracker">Mở Job Tracker</Link></div></div>
   ...
   <section id="st-output" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>8 · ST Output dm²/ngày — đọc số tổng bằng danh sách Job</b><small className="planning-sub">Training bắt buộc sau khi học Scheduling, vì output là báo cáo kế hoạch chứ không phải actual DONE.</small></div></div><div className="lg-body">
    <Module no="8A" title="Cách đọc ST Output" goal="Người mới biết số output đến từ nguồn nào và kiểm được từng dòng." steps={[
     ...
     {title:"Chọn Import History",detail:"Snapshot All Open Job được chọn dùng cho FINSST, CFINM-VN và ST Final Steps. Không tự lấy đại snapshot nếu đang training ngày khác."},
     {title:"Đọc tổng theo nguồn",detail:"CHEMMILL tính riêng mọi batch/flybar; công đoạn ST cuối chỉ tính batch đã dự kiến hoàn thành; FINSST/CFINM-VN lấy từ snapshot; ST Final Steps dùng candidate theo V523, sau đó chỉ giữ NextOperation thuộc active ST Scope: ST Only, Main Planning hoặc Intermediate."},
     {title:"Kiểm chống trùng",detail:"Final ST Operation Batch thắng FINSST/CFINM-VN; FINSST/CFINM-VN thắng ST Final Steps. CHEMMILL được tính độc lập nên có thể cùng Job với output cuối."},
    ...
    <div className="table-wrap"><table className="erp-table"><thead><tr><th>Nguồn</th><th>Khi nào Count = YES?</th><th>Điểm trainer phải hỏi học viên</th></tr></thead><tbody>
     ...
     <tr><td><b>ST Final Steps</b></td><td>Candidate được xác định theo logic V523, sau đó chỉ giữ Job có NextOperation thuộc active ST Scope: ST Only, Main Planning hoặc Intermediate, và chưa bị nguồn cao hơn lấy.</td><td>Operation ngoài 3 nhóm ST này không được tính.</td></tr>
    </tbody></table></div>
   </div></section>
