...
export default async function Page({searchParams}:{searchParams:Promise<{job?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page training-page">
   <div className="erp-page-head guide-head"><div><div className="erp-object-eyebrow">ONBOARDING · LIVE DATABASE · ST PLANNING · V521</div><h2>New User Training — học bằng dữ liệu thật đang chạy</h2><p>Lý thuyết trước, sau đó đối chiếu ngay Operation Code, Main, Area, Planner, Recipe, Batch Rules, Process Time, ST Output dm²/ngày và một Job thật từ database. V521 thêm workflow đọc Output theo kế hoạch: chọn Import History, kiểm cửa sổ Scheduled End 00:00 ngày D đến 03:00 ngày D+1, đọc tổng theo nguồn và drill-down từng Job. Các Alert/Audit và Batch/Schedule engine cũ vẫn giữ nguyên.</p></div><div className="erp-command-actions"><Link className="btn" href="/logic-guide">Logic & Hướng dẫn</Link><Link className="btn" href="/st-output">ST Output</Link><Link className="btn primary" href="/job-tracker">Mở Job Tracker</Link></div></div>
   ...
   <section id="flow" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>7 · Trainer dẫn học viên đi xuyên flow — từng câu hỏi phải trả lời</b></div></div><div className="lg-body"><ol className="lg-steps">
    ...
    <li><b>ST Output:</b> chọn ngày báo cáo và All Open Job Import History. Kiểm batch điều độ theo <code>Scheduled End</code> trong cửa sổ <b>00:00 D → 03:00 D+1</b>; đọc riêng CHEMMILL, công đoạn ST cuối, FINSST/CFINM-VN và INTERMEDIATE_NO_CHAIN. Mỗi dòng phải giải thích được vì sao Count = YES/NO.</li>
   ...
   <section id="st-output" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>8 · ST Output dm²/ngày — đọc số tổng bằng danh sách Job</b><small className="planning-sub">Training bắt buộc sau khi học Scheduling, vì output là báo cáo kế hoạch chứ không phải actual DONE.</small></div></div><div className="lg-body">
    <Module no="8A" title="Cách đọc ST Output" goal="Người mới biết số output đến từ nguồn nào và kiểm được từng dòng." steps={[
     ...
     {title:"Đọc tổng theo nguồn",detail:"CHEMMILL tính riêng mọi batch/flybar; công đoạn ST cuối chỉ tính batch đã dự kiến hoàn thành; FINSST/CFINM-VN và Intermediate No Chain lấy từ snapshot."},
    ...
    <div className="table-wrap"><table className="erp-table"><thead><tr><th>Nguồn</th><th>Khi nào Count = YES?</th><th>Điểm trainer phải hỏi học viên</th></tr></thead><tbody>
     ...
     <tr><td><b>Intermediate No Chain</b></td><td>Job thuộc audit Intermediate/Bridge ST chưa resolve Planning Chain, thêm điều kiện NextOperation thuộc ST, và chưa bị nguồn cao hơn lấy.</td><td>Đây là output cần kiểm audit, không phải lý do để tự ý tạo chain sai.</td></tr>
    </tbody></table></div>
   </div></section>
