...
export default async function Page({searchParams}:{searchParams:Promise<{job?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page training-page">
   <div className="erp-page-head guide-head"><div><div className="erp-object-eyebrow">ONBOARDING · LIVE DATABASE · ST PLANNING · V525</div><h2>New User Training — học bằng dữ liệu thật đang chạy</h2><p>Lý thuyết trước, sau đó đối chiếu ngay Operation Code, Main, Area, Planner, Recipe, Batch Rules, Process Time, ST Output dm²/ngày và một Job thật từ database. V525 giữ workflow Output theo kế hoạch: chọn Import History, kiểm cửa sổ Scheduled End 00:00 ngày D đến 03:00 ngày D+1, đọc tổng theo nguồn và drill-down từng Job; riêng INTERMEDIATE_NO_CHAIN phải lấy danh sách Audit All Open Job trước rồi lọc tiếp NextOperation có active ST Scope. Các Alert/Audit và Batch/Schedule engine cũ vẫn giữ nguyên.</p></div><div className="erp-command-actions"><Link className="btn" href="/logic-guide">Logic & Hướng dẫn</Link><Link className="btn" href="/st-output">ST Output</Link><Link className="btn primary" href="/job-tracker">Mở Job Tracker</Link></div></div>
   ...
   <section id="st-output" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>8 · ST Output dm²/ngày — đọc số tổng bằng danh sách Job</b><small className="planning-sub">Training bắt buộc sau khi học Scheduling, vì output là báo cáo kế hoạch chứ không phải actual DONE.</small></div></div><div className="lg-body">
    <Module no="8A" title="Cách đọc ST Output" goal="Người mới biết số output đến từ nguồn nào và kiểm được từng dòng." steps={[
     ...
     {title:"Đọc tổng theo nguồn",detail:"CHEMMILL tính riêng mọi batch/flybar; công đoạn ST cuối chỉ tính batch đã dự kiến hoàn thành; FINSST/CFINM-VN lấy từ snapshot; Intermediate No Chain lấy Audit All Open Job trước rồi chỉ giữ NextOperation có active ST Scope."},
    ...
    <div className="table-wrap"><table className="erp-table"><thead><tr><th>Nguồn</th><th>Khi nào Count = YES?</th><th>Điểm trainer phải hỏi học viên</th></tr></thead><tbody>
     ...
     <tr><td><b>Intermediate No Chain</b></td><td>Job nằm trong danh sách Audit All Open Job với reason INTERMEDIATE_NO_CHAIN, sau đó chỉ giữ NextOperation có active ST Scope, và chưa bị nguồn cao hơn lấy.</td><td>Đây là output cần kiểm audit, không phải lý do để tự ý tạo chain sai.</td></tr>
    </tbody></table></div>
   </div></section>
