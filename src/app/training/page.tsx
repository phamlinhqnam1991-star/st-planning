// V495 Training sync: Internal Chat is a shared Aiven-backed team channel. Operational commits also emit SYSTEM messages; cross-planner impacts are highlighted from dynamic Main → Schedule Area → Planner Assignment. Existing alerts remain canonical workflows.
...
export default async function Page({searchParams}:{searchParams:Promise<{job?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page training-page">
   <div className="erp-page-head guide-head"><div><div className="erp-object-eyebrow">ONBOARDING · LIVE DATABASE · ST PLANNING · V495</div><h2>New User Training — học bằng dữ liệu thật đang chạy</h2><p>Lý thuyết trước, sau đó đối chiếu ngay Operation Code, Main, Area, Planner, Recipe, Batch Rules, Process Time và một Job thật từ database. V495 giữ toàn bộ classifier READY/WAIT và Planning Board Quick View, đồng thời thêm Internal Chat dùng chung: mọi người chat với nhau và các thay đổi Planning/Scheduling/Production/Daily Adjustment được gửi thêm SYSTEM message. Khi thay đổi của Main thuộc Planner 1 tác động downstream Main của Planner 2 (hoặc ngược lại), Chat đánh dấu CROSS-PLANNER theo Planner Assignment đang chạy; các Alert/Audit cũ vẫn giữ nguyên.</p></div><div className="erp-command-actions"><Link className="btn" href="/logic-guide">Logic & Hướng dẫn</Link><Link className="btn primary" href="/job-tracker">Mở Job Tracker</Link></div></div>
   ...
   <section id="flow" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>7 · Trainer dẫn học viên đi xuyên flow — từng câu hỏi phải trả lời</b></div></div><div className="lg-body"><ol className="lg-steps">
    ...
    <li><b>Internal Chat:</b> mở tab Chat và gửi một tin nhắn test. Sau đó tạo/thêm Batch hoặc thay đổi Schedule ở Main test và kiểm tra SYSTEM message xuất hiện. Nếu downstream thuộc Planner khác, message phải có nhãn <b>CROSS-PLANNER</b> và hiển thị Planner nguồn → Planner bị ảnh hưởng. Chat chỉ thông báo; thao tác nghiệp vụ vẫn phải thực hiện ở Planning/Scheduling/Production.</li>
   ...
   <section id="scenarios" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>8 · Tất cả tình huống trainer phải đưa ra</b><small className="planning-sub">Không chỉ học happy path; phải biết vì sao sai và kiểm tra ở đâu.</small></div></div><div className="lg-body"><div className="table-wrap"><table className="erp-table"><thead><tr><th>Tình huống</th><th>Người mới phải hiểu</th><th>Cách xử lý đúng</th></tr></thead><tbody>
    ...
    <tr><td>Đọc Workload Summary trên Dashboard</td><td>Dashboard và Planning Board phải dùng cùng classifier cho cả READY và WAIT.</td><td>V486 tách READY thành 2 cột. V489 tách WAIT thành <b>WAIT · Next Main</b> và <b>WAIT · Future Mains</b>; từng cặp phải cộng lại đúng READY/WAIT tổng.</td></tr>
    <tr><td>Đọc Workload Summary trên Scheduling Board</td><td>Scheduling dùng cùng số liệu canonical nhưng trình bày để planner đọc handoff trước.</td><td>Đọc từ trái qua phải: <b>READY Scheduled/Done → READY not yet Schedule → WAIT Next Main → WAIT Future Mains → HOLD</b>. Riêng Scheduling Workload không cần hiển thị PLANNED-UNSCHEDULED, SCHEDULED hay Total. Hai READY dùng cùng kiểu card phẳng như WAIT: xanh lá đậm hơn = predecessor đã Scheduled/DONE; xanh lá nhạt hơn = predecessor mới Plan/Batch nhưng chưa Schedule. Flybar chỉ giữ Recipe rows. Mở breakdown dưới WAIT Next Main ở mọi khu vực, kể cả Chemical Line, để xem <b>Previous Main + Job + pcs + dm²</b>.</td></tr>
