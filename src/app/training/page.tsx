// V521 Training sync: add ST Output dm²/day workflow, Import History snapshot selection, scheduled-end window and drill-down dedupe explanation.
...
export default async function Page({searchParams}:{searchParams:Promise<{job?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page training-page">
   <div className="erp-page-head guide-head"><div><div className="erp-object-eyebrow">ONBOARDING · LIVE DATABASE · ST PLANNING · V524</div><h2>New User Training — học bằng dữ liệu thật đang chạy</h2><p>Lý thuyết trước, sau đó đối chiếu ngay Operation Code, Main, Area, Planner, Recipe, Batch Rules, Process Time, ST Output dm²/ngày và một Job thật từ database. V524 giữ workflow Output theo kế hoạch: chọn Import History, kiểm cửa sổ Scheduled End 00:00 ngày D đến 03:00 ngày D+1, đọc tổng theo nguồn và drill-down từng Job; riêng INTERMEDIATE_NO_CHAIN phải lấy danh sách audit trước rồi lọc tiếp NextOperation ST Planning có Source → Main Mapping. Các Alert/Audit và Batch/Schedule engine cũ vẫn giữ nguyên.</p></div><div className="erp-command-actions"><Link className="btn" href="/logic-guide">Logic & Hướng dẫn</Link><Link className="btn" href="/st-output">ST Output</Link><Link className="btn primary" href="/job-tracker">Mở Job Tracker</Link></div></div>

   <div className="guide-jump"><a href="#theory">1. Lý thuyết</a><a href="#live-config">2. Config thật</a><a href="#op-example">3. Operation Code thật</a><a href="#batch-rules">4. Batch Rules thật</a><a href="#time-rules">5. Process Time thật</a><a href="#job-live">6. Job thật</a><a href="#flow">7. Đi xuyên flow</a><a href="#st-output">8. ST Output</a><a href="#scenarios">9. Tình huống</a><a href="#security">10. Phân quyền</a><a href="#practice">11. Bài thực hành</a></div>
   ...
   <section id="theory" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>1 · Lý thuyết nền — đi từ chung nhất đến chi tiết nhất</b><small className="planning-sub">Người mới phải hiểu quan hệ này trước khi tạo Batch.</small></div></div><div className="lg-body">
    <div className="lg-key lg-key-2">
     ...
     <Card title="E. Output ST"><b>Schedule End + Import History → Output dm²/ngày</b>. Báo cáo output tính theo kế hoạch và luôn phải kiểm bằng bảng Job drill-down.</Card>
    </div>
   ...
   <section id="flow" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>7 · Trainer dẫn học viên đi xuyên flow — từng câu hỏi phải trả lời</b></div></div><div className="lg-body"><ol className="lg-steps">
    ...
    <li><b>ST Output:</b> chọn ngày báo cáo và All Open Job Import History. Kiểm batch điều độ theo <code>Scheduled End</code> trong cửa sổ <b>00:00 D → 03:00 D+1</b>; đọc riêng CHEMMILL, công đoạn ST cuối, FINSST/CFINM-VN và INTERMEDIATE_NO_CHAIN. Với INTERMEDIATE_NO_CHAIN, học viên phải kiểm được 2 bước: danh sách audit trước, rồi NextOperation phải là ST Planning Operation có Source → Main Mapping. Mỗi dòng phải giải thích được vì sao Count = YES/NO.</li>
   ...
   <section id="st-output" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>8 · ST Output dm²/ngày — đọc số tổng bằng danh sách Job</b><small className="planning-sub">Training bắt buộc sau khi học Scheduling, vì output là báo cáo kế hoạch chứ không phải actual DONE.</small></div></div><div className="lg-body">
    <Module no="8A" title="Cách đọc ST Output" goal="Người mới biết số output đến từ nguồn nào và kiểm được từng dòng." steps={[
     {title:"Chọn ngày báo cáo",detail:"Cửa sổ schedule là 00:00 ngày D đến 03:00 ngày D+1. Đây là rule riêng của Output ST, không thay Production Day 06:00 của Production Execution.",href:"/st-output"},
     {title:"Chọn Import History",detail:"Snapshot All Open Job được chọn dùng cho FINSST, CFINM-VN và Intermediate No Chain. Không tự lấy đại snapshot nếu đang training ngày khác."},
     {title:"Đọc tổng theo nguồn",detail:"CHEMMILL tính riêng mọi batch/flybar; công đoạn ST cuối chỉ tính batch đã dự kiến hoàn thành; FINSST/CFINM-VN lấy từ snapshot; Intermediate No Chain lấy audit list trước rồi chỉ giữ NextOperation thuộc ST Planning có Source -> Main Mapping."},
     {title:"Kiểm chống trùng",detail:"Final ST Operation Batch thắng FINSST/CFINM-VN; FINSST/CFINM-VN thắng Intermediate No Chain. CHEMMILL được tính độc lập nên có thể cùng Job với output cuối."},
     {title:"Drill-down Job",detail:"Mỗi dòng phải xem được Job, Part, Revision, Qty, Surface, Total dm², NextOperation, Final ST Operation, Batch, Area, Resource, Scheduled End, Import và Audit Reason."}
    ]}/>
    <div className="table-wrap"><table className="erp-table"><thead><tr><th>Nguồn</th><th>Khi nào Count = YES?</th><th>Điểm trainer phải hỏi học viên</th></tr></thead><tbody>
     <tr><td><b>CHEMMILL</b></td><td>Batch/flybar CHEMMILL có Scheduled End trong cửa sổ output.</td><td>Vì sao Job này vẫn có thể được tính thêm ở công đoạn cuối?</td></tr>
     <tr><td><b>Công đoạn ST cuối</b></td><td>Batch của Final ST Operation có Scheduled End trong cửa sổ output.</td><td>Final ST Operation lấy từ Planning Chain hay fallback AllOperation?</td></tr>
     <tr><td><b>FINSST / CFINM-VN</b></td><td>Job trong snapshot được chọn có NextOperation là FINSST hoặc CFINM-VN, và chưa bị nguồn ưu tiên cao hơn lấy.</td><td>Snapshot đang chọn có đúng ngày/import cần báo cáo không?</td></tr>
     <tr><td><b>Intermediate No Chain</b></td><td>Job nằm trong danh sách audit Intermediate/Bridge chưa resolve current Planning Board row, sau đó qua bước lọc NextOperation phải là ST Planning Operation có active Source → Main Mapping, và chưa bị nguồn cao hơn lấy.</td><td>Đây là output cần kiểm audit, không phải lý do để tự ý tạo chain sai.</td></tr>
    </tbody></table></div>
   </div></section>

   <section id="scenarios" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>9 · Tất cả tình huống trainer phải đưa ra</b><small className="planning-sub">Không chỉ học happy path; phải biết vì sao sai và kiểm tra ở đâu.</small></div></div><div className="lg-body"><div className="table-wrap"><table className="erp-table"><thead><tr><th>Tình huống</th><th>Người mới phải hiểu</th><th>Cách xử lý đúng</th></tr></thead><tbody>
    ...
    <tr><td>Đọc Output ST dm²/ngày</td><td>Đây là output theo kế hoạch, không phải actual Production DONE.</td><td>Chọn đúng ngày + Import History, kiểm Window 00:00 D → 03:00 D+1, rồi đọc Job detail để xác nhận nguồn và Count YES/NO.</td></tr>
    <tr><td>Job vừa có CHEMMILL vừa có công đoạn cuối</td><td>CHEMMILL là output flybar độc lập, không bị rule chống trùng của final output.</td><td>Cho phép cả hai dòng Count = YES nếu đều thỏa điều kiện; không xóa dòng CHEMMILL.</td></tr>
    <tr><td>FINSST và CFINM-VN</td><td>Cả hai NextOperation đều thuộc nhóm đã ra khỏi ST theo snapshot được chọn.</td><td>Nếu cùng Job cũng có Final ST Operation Batch trong cửa sổ, ưu tiên batch công đoạn cuối và loại dòng snapshot là duplicate.</td></tr>
   ...
   <section id="practice" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>11 · Bài thực hành bắt buộc trước khi dùng app thật</b></div></div><div className="lg-body"><ol className="lg-steps">
    ...
    <li><b>Tính Output thử:</b> mở ST Output, chọn đúng ngày/import, tìm Job vừa schedule và giải thích Total dm² + Output Source + Count YES/NO.</li>
