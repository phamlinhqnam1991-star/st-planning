// V492 Training sync: Scheduling workload cards open Planning Board Quick View; READY can create/add Batch through the canonical Planning API, WAIT/HOLD stay read-only, and the popup teaches Next Main + Next Recipe No/Name.
...
export default async function Page({searchParams}:{searchParams:Promise<{job?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page training-page">
   <div className="erp-page-head guide-head"><div><div className="erp-object-eyebrow">ONBOARDING · LIVE DATABASE · ST PLANNING · V492</div><h2>New User Training — học bằng dữ liệu thật đang chạy</h2><p>Lý thuyết trước, sau đó đối chiếu ngay Operation Code, Main, Area, Planner, Recipe, Batch Rules, Process Time và một Job thật từ database. V492 giữ classifier READY/WAIT của Planning Board và thêm Planning Board Quick View ngay từ Scheduling Workload: click card để xem đúng Job theo Main/Recipe/Bucket; popup có Next Main + Next Recipe No/Name. READY được tạo/thêm Batch bằng canonical Batch API, WAIT/HOLD chỉ xem.</p></div><div className="erp-command-actions"><Link className="btn" href="/logic-guide">Logic & Hướng dẫn</Link><Link className="btn primary" href="/job-tracker">Mở Job Tracker</Link></div></div>
   ...
   <section id="flow" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>7 · Trainer dẫn học viên đi xuyên flow — từng câu hỏi phải trả lời</b></div></div><div className="lg-body"><ol className="lg-steps">
    ...
    <li><b>Planning Board:</b> Job READY, WAIT · Next Main hay WAIT · Future Mains? Từ V492 có thể click card Workload ngay tại Scheduling để mở <b>Planning Board Quick View</b>. READY cho phép chọn Job và Add/Create Batch qua chính Batch engine của Planning Board; WAIT/HOLD chỉ xem. Popup hiển thị cả <b>Next Main + Next Recipe No/Name</b>. WAIT · Next Main là blocker gần nhất sau frontier; WAIT · Future Mains là các Main xa hơn. Nếu cùng Job/Main nhiều batch thì Planning Board hiển thị nối bằng &.</li>
