// V489 Training sync: WAIT is split into immediate Next Main versus later Future Mains while internal LOCKED logic remains unchanged.
...
export default async function Page({searchParams}:{searchParams:Promise<{job?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page training-page">
   <div className="erp-page-head guide-head"><div><div className="erp-object-eyebrow">ONBOARDING · LIVE DATABASE · ST PLANNING · V490</div><h2>New User Training — học bằng dữ liệu thật đang chạy</h2><p>Lý thuyết trước, sau đó đối chiếu ngay Operation Code, Main, Area, Planner, Recipe, Batch Rules, Process Time và một Job thật từ database. V490 dùng cùng classifier READY/WAIT với Planning Board và bổ sung cách đọc Scheduling Workload: READY-first, màu xanh handoff, Flybar recipe-only và WAIT Next Main breakdown ngoài Chemical Line.</p></div><div className="erp-command-actions"><Link className="btn" href="/logic-guide">Logic & Hướng dẫn</Link><Link className="btn primary" href="/job-tracker">Mở Job Tracker</Link></div></div>
   ...
   <section id="scenarios" className="erp-table-panel guide-section"><div className="erp-panel-head"><div><b>8 · Tất cả tình huống trainer phải đưa ra</b><small className="planning-sub">Không chỉ học happy path; phải biết vì sao sai và kiểm tra ở đâu.</small></div></div><div className="lg-body"><div className="table-wrap"><table className="erp-table"><thead><tr><th>Tình huống</th><th>Người mới phải hiểu</th><th>Cách xử lý đúng</th></tr></thead><tbody>
    ...
    <tr><td>Đọc Workload Summary trên Scheduling Board</td><td>Scheduling dùng cùng số liệu canonical nhưng trình bày để planner đọc handoff trước.</td><td>Đọc từ trái qua phải: <b>READY Scheduled/Done → READY not yet Schedule → WAIT Next Main → WAIT Future Mains → PLANNED-UNSCHEDULED → SCHEDULED → HOLD</b>. Xanh lá đậm = predecessor đã Scheduled/DONE; xanh lá nhạt = predecessor mới Plan/Batch nhưng chưa Schedule. Flybar chỉ giữ Recipe rows. Ngoài Chemical Line, mở breakdown dưới WAIT Next Main để xem Job đang chờ từ immediate Previous Main nào.</td></tr>
