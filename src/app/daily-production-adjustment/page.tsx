...
export default async function Page({searchParams}:{searchParams:Promise<{date?:string}>}){
 ...
 return <main className="erp-shell erpkit-migrated-page"><ErpAppHeader module="DAILY PRODUCTION ADJUSTMENT"/><AppTabs active="adjustment"/>
  <section className="erp-content erp-content-full"><div className="erp-page-head"><div><div className="erp-object-eyebrow">OPERATIONS · RECONCILIATION</div><h2>Daily Production Adjustment</h2><p>Đối soát Production Report trước 05:59 → Carry Over, bớt Job chưa làm, thêm Job ngoài lô và cascade lịch qua Main/Planner/Resource.</p></div><div className="production-date-nav"><Link className="btn" href={`/daily-production-adjustment?date=${shift(date,-1)}`}>‹ Previous</Link><span>{display(date)}</span><Link className="btn" href={`/daily-production-adjustment?date=${shift(date,1)}`}>Next ›</Link>{date!==current?<Link className="btn primary" href={`/daily-production-adjustment?date=${current}`}>Today</Link>:null}</div></div>
