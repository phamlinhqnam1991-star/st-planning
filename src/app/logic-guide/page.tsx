  ...
  <section className="erp-content erp-content-full guide-page">
   ...
   <Section id="schedule" title="9 · Tab Board Điều Độ — xếp Batch vào resource và thời gian"
    ...
    <Rule title="Khóa Previous Main khi THÊM Batch vào điều độ" tone="important">
     Chỉ khi <b>thêm một existing Planning Batch</b> vào Board Điều Độ, server kiểm tra vật lý theo từng Job trong Batch. Nếu Job có <b>Immediate Previous Main</b> thì Previous Main đó phải đã có Schedule hợp lệ (không CANCELLED và có <code>planned_end</code>); đồng thời <b>Start của Current Main phải ≥ End của Previous Main</b>. Nếu bất kỳ Job nào chưa có Previous Main Schedule hoặc Current Start còn sớm hơn Previous End, toàn bộ thao tác Save bị khóa và không insert Schedule. Main đầu tiên không có Previous Main được phép điều độ bình thường. Đây là <b>Scheduling add-only guard</b>, không thay Sequential READY/WAIT của Planning Board: READY vẫn có thể mở khi Previous Main mới chỉ có Batch UNSCHEDULED. Edit/PATCH lịch đã có và Trial Day Shift không dùng guard này theo phạm vi đã chốt. Với <b>Chemical Line</b>, engine Đề xuất/FB/Loading/Process/NDT/Unloading hiện tại giữ nguyên; server chạy logic đề xuất/capacity hiện hữu trước, sau đó chỉ kiểm tra <b>effective Start cuối cùng</b> ngay trước khi Save nên không thay công thức đề xuất Chemical Line.
    </Rule>
    <Rule title="Trial · Dời toàn bộ ngày điều độ" tone="important">
     Nút <b>Dời toàn bộ lịch → +1 ngày</b> trên Board Điều Độ dùng để tiếp tục một ngày test sang ngày kế tiếp. Hệ thống <b>MOVE in-place</b> toàn bộ Schedule của ngày đang xem, không clone Batch và không tạo Schedule thứ hai; vì vậy sau commit <b>ngày nguồn phải rỗng</b>. Resource, Recipe, Duration, Sequence và toàn bộ timestamp của Chemical Line <b>Loading / Process / NDT / Unloading</b> được dịch cùng đúng ±1 ngày. <b>← Lùi 1 ngày</b> dùng như thao tác hoàn tác trong mô hình trial một ngày. Cả hai thao tác chạy trong một transaction; nếu ngày đích đã có lịch độc lập, có RUNNING/COMPLETED hoặc có lịch khác chạy xuyên khoảng đích thì rollback toàn bộ, không tự xóa lịch ngày đích và không merge hai ngày. Planning Chain/Candidate/Batch membership/Recipe không thay đổi.
    </Rule>
