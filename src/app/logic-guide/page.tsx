// V496 Dashboard/Scheduling workload presentation: Dashboard Area workload now mirrors Planning Board column order/tone and removes KPI cards; Scheduling READY recipe rows show Previous Main Recipe breakdown while MAIN TOTAL stays compact.
  ...
  <section className="erp-content erp-content-full guide-page">
   ...
   <Section id="quick" title="1 · Bắt đầu nhanh — thứ tự vận hành chuẩn"
    ...
    <div className="lg-key lg-key-2">
     ...
     <Rule title="V496 · Đồng bộ Workload Dashboard + READY Recipe Breakdown" tone="important">
      <b>Dashboard</b> bỏ toàn bộ KPI card và giữ bảng workload chi tiết. Bảng <b>ST Workload Summary · By Area</b> dùng đúng thứ tự cột và ngôn ngữ màu của Planning Board: WAIT Next → WAIT Future → READY Previous Main Scheduled → READY Previous Main Unscheduled / START → Planned-Unscheduled → Scheduled → Hold → ST Only → Total. <b>Scheduling Board</b> giữ layout V491 nhưng ở hai cột READY, mỗi <b>Recipe row</b> có breakdown theo <b>Recipe của Previous Main</b> (Previous Main + Recipe No + Job + pcs + dm²). Dòng <b>MAIN TOTAL</b> không breakdown để tránh rối. Đây chỉ là presentation/read model; READY/WAIT gating và Batch/Schedule engine không đổi.
     </Rule>
    </div>
...
<Rule title="Hiệu năng tải trang · V437">Resolver không quét/rebuild toàn bộ <code>md_routing_detailed</code> khi mở tab. Hệ thống lọc trước Batch/Job thuộc đúng view/ngày đang xem, lấy danh sách Part/Revision liên quan, rồi mới dựng Routing Main + Masking/Unmasking cho tập nhỏ đó. Đây là tối ưu query/index, không đổi boundary Previous Main → Current Main hay logic support.</Rule>
   ...
   <Section id="schedule" title="9 · Tab Scheduling Board — xếp Batch vào resource và thời gian"
    ...
    <Rule title="ST Workload Summary · By Area trên Scheduling Board" tone="important">
     ...
     <br/><br/><b>V496 READY recipe breakdown:</b> ở hai cột <b>READY · Previous Main Scheduled / Done</b> và <b>READY · Previous Main Not Yet Scheduled</b>, mỗi <b>Recipe row</b> hiển thị thêm breakdown theo <b>Recipe của Previous Main</b>: <b>← Previous Main · Previous Recipe No · Job · pcs · dm²</b> (Recipe Name nằm trong tooltip/title). Dòng <b>MAIN TOTAL</b> chỉ giữ metric tổng, không render breakdown. Với First Main/START không có Previous Main nên không có previous-recipe breakdown.
