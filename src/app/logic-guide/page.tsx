// V492 Scheduling Workload Quick View: clickable workload cards open a Planning Board popup filtered by Area/Main/Recipe/Bucket; READY can create/add Batch through the canonical Planning Batch API, WAIT/HOLD stay read-only. Popup also shows Next Main + Next Recipe No/Name.
  ...
  <section className="erp-content erp-content-full guide-page">
   <div className="erp-page-head guide-head">
    <div>
     ...
     <p>Flow · Mapping · Cách thao tác · Ảnh hưởng phía sau theo logic hiện tại đến V492.</p>
    </div>
   </div>
...
<Rule title="Hiệu năng tải trang · V437">Resolver không quét/rebuild toàn bộ <code>md_routing_detailed</code> khi mở tab. Hệ thống lọc trước Batch/Job thuộc đúng view/ngày đang xem, lấy danh sách Part/Revision liên quan, rồi mới dựng Routing Main + Masking/Unmasking cho tập nhỏ đó. Đây là tối ưu query/index, không đổi boundary Previous Main → Current Main hay logic support.</Rule>
   ...
   <Section id="schedule" title="9 · Tab Scheduling Board — xếp Batch vào resource và thời gian"
    ...
    <Rule title="ST Workload Summary · By Area trên Scheduling Board" tone="important">
     ...
     <br/><br/><b>V492 Planning Board Quick View:</b> click vào card workload mở popup dùng đúng canonical Dashboard/Planning Chain population và lọc theo <b>Area + Main + Recipe + Bucket</b>; click riêng dòng breakdown WAIT còn lọc thêm <b>Previous Main</b>. Popup hiển thị Job/Part/Rev/Qty/dm²/Priority/Previous Main/Main/Recipe và thêm <b>Next Main + Next Recipe No + Next Recipe Name</b> để thấy bước kế tiếp ngay khi chọn Job. Chỉ hai bucket READY được chọn Job để <b>Add to Existing Batch</b> hoặc <b>Create New Batch</b>, và mutation gọi lại chính <code>/api/planning/batch</code> nên không có Batch engine thứ hai. WAIT/HOLD chỉ xem, không bypass gating. Sau khi tạo/thêm Batch, workload popup và Scheduling workload refresh lại từ server; Main kế tiếp mở theo rule hiện hữu.
   ...
   <section className="erp-table-panel guide-section">
    <div className="erp-panel-head"><div><b>New User Training · V492 · Live Database</b><small className="planning-sub">Training và Logic & Hướng dẫn được cập nhật song song. V492 bổ sung Planning Board Quick View từ Scheduling Workload, vẫn dùng classifier WAIT V489 và canonical Batch engine; popup có Next Main + Next Recipe.</small></div></div>
