// V527 ST Output: V523 candidate + exact active ST-scope filter for ST Final Steps.
// Intermediate/Bridge ST without resolved Planning Chain + NextOperation in the legacy V523 ST Output scope.
 ...
 return <main className="erp-shell erpkit-migrated-page">
  ...
  <section className="erp-content erp-content-full guide-page">
   ...
   <div className="guide-jump">
    ...
    <a href="#daily-production-adjustment">Daily Production Adjustment</a>
    <a href="#production-change-alerts">Production Change Alerts</a>
    <a href="/training">New User Training</a>
   ...
   <Section id="quick" title="1 · Bắt đầu nhanh — thứ tự vận hành chuẩn"
    sub="Nếu hệ thống đã cấu hình xong, người dùng hằng ngày chủ yếu đi từ All Open Jobs → Planning Board → Scheduling Board">
    <Chain steps={[
     ...
     {t:"E · Scheduling Board",d:"Unscheduled Batch → Resource/Time",c:"green"},
    ...
    <div className="lg-key lg-key-2">
     ...
     <Rule title="V510 · Internal Chat realtime + unread + direct user" tone="important">
      Tab <b>Internal Chat</b> dùng Aiven PostgreSQL và lớp <b>Global Realtime No-Supabase V509</b>. Mọi thay đổi đã commit ở Planning Batch/Batch Job, Điều độ, Production và Daily Adjustment tiếp tục sinh <b>SYSTEM message</b>; riêng mỗi Chat message còn phát một domain <b>CHAT</b> vào <code>system_change_event</code> để các máy/tab cập nhật ngay mà không F5. Tab Chat hiển thị badge tổng tin chưa đọc; trong Chat có <b>ST Planning Group</b> và danh sách active user để chọn chat trực tiếp, mỗi user có unread riêng. Read state tách theo conversation nên đọc Group không làm mất unread Direct Message. Lỗi Chat/realtime phải fail-open và không được rollback Planning/Schedule/Production. Chạy migration <code>087_internal_chat_direct_realtime.sql</code> sau migration 086.
     </Rule>
     ...
     <Rule title="V519 · Masking Time nhỏ hơn font số lô" tone="important">
      Trong bảng Điều độ, dòng <b>Masking time HH:MM</b> giữ một dòng gọn nhưng dùng font 8px, nhỏ hơn font cơ sở 9px của số lô. Đây chỉ là thay đổi UI; công thức Masking V512 và mọi logic Scheduling không đổi.
     </Rule>
     ...
     <Rule title="V526 · ST Output dm²/ngày theo kế hoạch · cách tính V523" tone="important">
      Tab <b>Operations → ST Output</b> tính output toàn ST theo ngày báo cáo. Batch đã điều độ dùng cửa sổ <b>00:00 ngày D → 03:00 ngày D+1</b> theo <code>Scheduled End</code>. <b>CHEMMILL</b> tính riêng tất cả flybar/batch trong cửa sổ, không cần là công đoạn cuối. Các batch khác chỉ tính khi đó là <b>công đoạn ST cuối</b>, ưu tiên <code>Planning Chain</code> và fallback <code>AllOperation</code>. Nhóm snapshot lấy từ <b>Import History</b> gồm <code>FINSST</code>, <code>CFINM-VN</code> và <b>ST Final Steps</b>. ST Final Steps giữ cách xác định candidate theo V523, sau đó lọc lần cuối theo <code>NextOperation</code>: chỉ tính operation nằm trong active ST Scope thuộc một trong ba loại <code>ST_SCOPE_ONLY</code>, <code>PLANNING_OPERATION</code> hoặc <code>INTERMEDIATE</code>. Operation ngoài ba nhóm này không được tính. Chống trùng theo ưu tiên <b>Final ST Operation Batch → FINSST/CFINM-VN → ST Final Steps</b>; CHEMMILL là output độc lập. Màn hình luôn có bảng Job drill-down để thấy Qty, Surface, Batch, Schedule End, Import và lý do tính/loại.
     </Rule>
     ...
     <Rule title="V512 · Masking Time Estimate hỗ trợ Điều độ" tone="important">
      Masking Time là <b>advisory</b>, không phải Main Operation hay resource điều độ. Config cho phép khai báo <b>tổng số người Masking</b>, phân bổ người theo <b>Physical Area</b> và gán <b>Main Operation → cột thời gian thật trong All Open Job</b> từ Open Job Column Values. Mỗi mapping chọn <b>JOB_TOTAL/PER_PIECE</b> và <b>HOURS/MINUTES</b>. Hệ thống cộng person-hours của các Job trong Batch, chia số người để ra <b>Estimated Masking Duration</b>; nếu biết Previous Main planned_end thì hiển thị <b>Estimated Masking Ready</b>. Nếu Schedule Start sớm hơn Ready thì đánh dấu <b>MASKING NOT READY</b> nhưng không chặn Save/Start. Dữ liệu trống/sai format chỉ cảnh báo. Migration <code>088_masking_time_estimate_advisory.sql</code> là schema bổ sung; thiếu migration thì Điều độ vẫn fail-open và tải bình thường.
     </Rule>
   ...
   <Section id="flow" title="2 · Flow dữ liệu & dependency tổng thể"
    ...
    <StepList items={[
     ...
     <>Nếu Operation là <b>ST_SCOPE_ONLY</b>, Job vẫn thấy ở All Open Jobs nhưng Operation đó không trở thành Main Planning, không tạo Batch và không vào Scheduling Board.</>,
   ...
   <Section id="config" title="4 · Tab Cấu hình — trình tự và ảnh hưởng downstream"
    ...
    <details className="erp-details">
     ...
     <div className="notice"><b>Impact:</b> PLANNING_OPERATION và ST_SCOPE_ONLY vẫn là cấu hình operational upstream. Riêng <b>INTERMEDIATE Dashboard ST</b> chỉ ảnh hưởng population của toàn bộ Dashboard và không kích hoạt Planning Chain sync. Mapping sai Area/Planner của Planning Operation vẫn ảnh hưởng Scheduling Board.</div>
    </details>
    ...
    <details className="erp-details">
     ...
     <Rule title="Area Display Order · V473">Trong Configuration có tab riêng <b>Area Display Order</b>. Planner dùng ↑/↓ để đặt thứ tự Physical Area; hệ thống ghi vào <code>md_area.sort_order</code>. Thứ tự này là nguồn hiển thị cho Dashboard/Planning workload và các report panel có grouping theo Area. Không đổi mapping ST Group, Schedule Area, Resource hay Planner.</Rule></details>

    <details className="erp-details">
     <summary><b>⑦ Khu vực điều độ (Schedule Area / Lane)</b></summary>
     <p>Định nghĩa lane trên Scheduling Board: tên, thứ tự, resource group/resource, số dòng mặc định, các Main Operation được phép vào lane và trạng thái Điều độ tay.</p>
    ...
    <details className="erp-details">
     ...
     <p>Gán Schedule Area cho Planner 1/2. Scheduling Board lấy ownership động từ mapping này.</p>
    ...
    <div className="lg-subtitle">4.2 · Tầng 2 — Rules & Formula</div>
    <details open className="erp-details">
     <summary><b>⑨ Rules & Formula — Recipe · Công đoạn · Mã lô</b></summary>
   ...
   <Section id="jobtracker" title="5B · Tab Job Tracker — tra toàn bộ vòng đời một Job"
    ...
    <div className="lg-key lg-key-2">
     <Rule title="Job Tracker không thay đổi dữ liệu">Tab này chỉ đọc/đối chiếu. Muốn sửa Recipe/Mapping/Time/Area phải quay về Cấu hình; muốn sửa Batch vào Planning Board; muốn sửa giờ/resource vào Scheduling Board.</Rule>
   ...
   <Section id="st-output" title="6.5 · Tab ST Output — Output ST dm²/ngày theo kế hoạch"
    ...
    <StepList items={[
     ...
     <>Chọn <b>All Open Job Import History</b> để làm snapshot cho các Job đã ra khỏi ST bằng <code>FINSST</code> hoặc <code>CFINM-VN</code>, và cho nhóm <b>ST Final Steps</b>.</>,
     <><b>ST Final Steps</b>: bước 1 giữ candidate theo V523; bước 2 chỉ giữ <code>NextOperation</code> có active ST Scope loại <code>ST_SCOPE_ONLY</code>, <code>PLANNING_OPERATION</code> hoặc <code>INTERMEDIATE</code>. Không nằm trong ba nhóm này thì loại khỏi ST Output.</>,
     <>Đọc tổng <b>Total Output dm²</b>, sau đó đối chiếu breakdown theo nguồn: <b>CHEMMILL</b>, <b>Công đoạn ST cuối</b>, <b>FINSST/CFINM-VN</b>, <b>ST Final Steps</b>.</>,
     ...
     <>Nếu một Job xuất hiện ở nhiều nguồn, chỉ dòng ưu tiên cao nhất được tính: <b>Final ST Operation Batch → FINSST/CFINM-VN → ST Final Steps</b>. Dòng trùng vẫn xem được khi chọn trạng thái <b>Tất cả dòng audit</b> hoặc <b>Bị loại/trùng</b>.</>,
   ...
   <Section id="masking" title="8 · Masking / Unmasking — logic support dùng trong Báo cáo sản xuất"
    ...
    <StepList items={[
     ...
     <>Một ngày chuẩn là <b>06:00 ngày D → trước 06:00 ngày D+1</b>; mọi Batch có <code>planned_start</code> nằm trong cửa sổ này thuộc ngày D, kể cả Start 00:00–05:59 của ngày lịch kế tiếp. Masking/Unmasking trong Báo cáo sản xuất dùng đúng cùng boundary với Scheduling Board.</>,
     ...
     <>Khi planner đổi ngày/giờ/resource trên Scheduling Board, Job support trong Báo cáo sản xuất tự chuyển ngày và cập nhật Start/End theo dữ liệu schedule mới ở lần mở/refresh tiếp theo.</>,
...
<Rule title="Hiệu năng tải trang · V437">Resolver không quét/rebuild toàn bộ <code>md_routing_detailed</code> khi mở tab. Hệ thống lọc trước Batch/Job thuộc đúng view/ngày đang xem, lấy danh sách Part/Revision liên quan, rồi mới dựng Routing Main + Masking/Unmasking cho tập nhỏ đó. Đây là tối ưu query/index, không đổi boundary Previous Main → Current Main hay logic support.</Rule>
   ...
   <Section id="schedule" title="9 · Tab Scheduling Board — xếp Batch vào resource và thời gian"
    ...
    <Rule title="ST Workload Summary · By Area trên Scheduling Board" tone="important">
     ...
     <br/><br/><b>V499 detail-line Recipe click (thay vị trí V498):</b> <b>MAIN TOTAL</b> trở lại chỉ hiển thị tổng, không có Recipe picker. Planner click trực tiếp vào <b>dòng detail của Recipe row</b>; với READY, dòng <b>→ Current Main · Current Recipe</b> mở Planning Board Quick View đã lọc <b>Area + Main + Recipe + READY bucket</b>, cùng kiểu thao tác trực tiếp như breakdown của <b>WAIT · Next Main</b>. Đây chỉ là presentation/navigation, không đổi READY/WAIT, Recipe resolver, Batch hay Schedule.
     <br/><br/><b>V500 Scheduling detail-only + Dashboard READY-first:</b> trên <b>Tab Điều độ</b>, xóa toàn bộ dòng <b>MAIN TOTAL</b> trong ST Workload Summary; mỗi Recipe detail row hiển thị trực tiếp Main Operation để không mất ngữ cảnh và vẫn giữ click Quick View của V499. Trên <b>Dashboard · ST Workload Summary · By Area</b>, thứ tự cột là <b>READY Previous Main Scheduled → READY Previous Main Unscheduled / START → WAIT Next Main → WAIT Future Mains → PLANNED-UNSCHEDULED → SCHEDULED → HOLD → ST ONLY → Total</b>. Đây chỉ là presentation; workload population/classifier và Batch/Schedule engine không đổi.
     <br/><br/><b>V493 Planning Board Quick View:</b> click vào card workload mở popup dùng đúng canonical Dashboard/Planning Chain population và lọc theo <b>Area + Main + Recipe + Bucket</b>; click riêng dòng breakdown WAIT còn lọc thêm <b>Previous Main</b>. Popup hiển thị Job/Part/Rev/Qty/dm²/Priority/Previous Main/Main/Recipe và thêm <b>Next Main + Next Recipe No + Next Recipe Name</b> để thấy bước kế tiếp ngay khi chọn Job. Chỉ hai bucket READY được chọn Job để <b>Add to Existing Batch</b> hoặc <b>Create New Batch</b>, và mutation gọi lại chính <code>/api/planning/batch</code> nên không có Batch engine thứ hai. WAIT/HOLD chỉ xem, không bypass gating. V493 thêm nút lọc ngay trên từng cột của popup; có thể giữ đồng thời nhiều bộ lọc và Clear filters để trở về toàn bộ card. Check-all chỉ tác động lên các Job đang hiển thị sau lọc, còn Batch mutation vẫn dùng selection thật và canonical API. Sau khi tạo/thêm Batch, workload popup và Scheduling workload refresh lại từ server; Main kế tiếp mở theo rule hiện hữu.
     <br/><br/><b>V497/V499 READY recipe detail:</b> cột <b>READY · Previous Main Not Yet Scheduled</b> tiếp tục hiển thị <b>→ Current READY Main · Current READY Recipe · Job · pcs · dm²</b>. <b>V503</b> thay riêng cột <b>READY · Previous Main Scheduled / Done</b>: detail được regroup theo <b>Next Main + Recipe của Next Main</b> và hiển thị <b>→ Next Main · Next Recipe · Job · pcs · dm²</b>. Click detail V503 mở Quick View đúng Current Main/Current Recipe/READY bucket và lọc thêm đúng Next Main/Next Recipe của dòng đã click. Đây chỉ là presentation/navigation; READY/WAIT classifier, workload total, Recipe resolver, Batch và Schedule không đổi. Từ <b>V500</b>, Scheduling Board không còn bất kỳ dòng <b>MAIN TOTAL</b> nào.
   ...
   <Section id="production" title="11 · Production Execution — báo cáo thực tế theo Batch (đến V469)"
    ...
    <div className="lg-key lg-key-2">
     ...
     <Rule title="Report panel grouping · V473" tone="important">Sub-tab <b>Masking &amp; Unmasking</b> vẫn lấy Physical Area của Main Planning liên kết, nhưng <b>không còn gộp tất cả Main Painting vào một bảng</b>. V473 tách Preparation thành panel riêng theo <b>Area + Main</b>: <b>PRIMER, PRIMER2, PRIMER3, TOPCOAT1, TOPCOAT2, ANTI-ABRASION</b> (và Main khác nếu có). Ví dụ sẽ thấy <code>Painting · PRIMER (Preparation)</code> và <code>Painting · TOPCOAT1 (Preparation)</code> riêng. Mỗi Job vẫn gộp step theo thứ tự Unmasking → Masking; execution status từng support step không đổi. Sub-tab <b>Painting</b> Main work vẫn giữ 4 panel <b>CAB1/CAB2/CAB3/Powercoating</b>.</Rule>
   ...
   <Section id="impact" title="13 · Impact Matrix — sửa ở đâu thì phía sau thay đổi gì?"
    ...
    <div className="table-wrap"><table className="erp-table">
     ...
     <tbody>
      ...
      <tr><td>Schedule Area</td><td>Lane, Main và danh sách Recipe được phép chọn trên Scheduling Board.</td><td>Unscheduled Batch xuất hiện/kéo được ở đâu; Recipe dropdown chỉ lấy Recipe map tới Main của area.</td><td>Kiểm tra Main mapping + Recipe mapping + Planner assignment.</td></tr>
   ...
   <Section id="database-provider" title="15 · Database Provider · Aiven PostgreSQL (V438)"
    ...
    <StepList items={[
     ...
     <>Từ <b>V509</b>, Global Realtime No-Supabase dùng mô hình <b>fail-safe leader</b>: chỉ một tab đang visible trên mỗi browser profile poll <code>system_change_event</code> khoảng 1.8 giây/lần; các tab cùng máy nhận qua <code>BroadcastChannel/localStorage</code>. Nếu realtime API/DB lỗi hoặc migration 086 chưa chạy, trang vẫn phải mở bình thường và feed tự backoff/retry; không F5 và không reload document.</>,
     <>Từ <b>V510</b>, Internal Chat dùng cùng feed PostgreSQL: SYSTEM message từ Planning/Điều độ/Production phát domain CHAT ngay sau commit; unread badge và Direct Chat giữa các active user dùng migration <code>087_internal_chat_direct_realtime.sql</code>.</>,
    ...
    <Rule title="Global Realtime No-Supabase · V509/V510" tone="important">Chạy migration <code>086_global_realtime_change_event.sql</code>; riêng Internal Chat V510 chạy thêm <code>087_internal_chat_direct_realtime.sql</code>. Chỉ leader tab poll PostgreSQL; tab khác nhận local broadcast. Internal Chat phát domain CHAT cho system/user messages và cả read-receipt; riêng PATCH chỉ làm mới unread state, không reload message nên tránh feedback loop. Realtime lỗi phải fail-open: app vẫn dùng được, không F5, không reload document.</Rule>
   ...
   <Section id="production-change-alerts" title="17 · Cảnh báo thay đổi bởi Sản xuất (V468–V511)"
    sub="Production Add Job và Remove Before Start đều để lại audit + downstream impact; Shift xử lý Remove downstream tại Báo cáo sản xuất, Điều độ chỉ theo dõi trạng thái">
    <div className="lg-key lg-key-2">
     ...
     <Rule title="Downstream Shift Accept · V511" tone="important">Job bị untick không bị xóa âm thầm khỏi lô Main sau. Hệ thống tạo <b>REMOVE_JOB Handover Event</b> cho từng downstream Batch đang chứa Job. <b>Báo cáo sản xuất</b> của Batch bị ảnh hưởng hiển thị <b>Upstream Impact · Shift Action</b>; Shift Supervisor bấm <b>Accept & Remove Job</b> tại đây để loại Job và recompute Qty/Surface/Process Time. <b>Điều độ chỉ hiển thị WAITING SHIFT ACCEPT / ACCEPTED / CRITICAL và link sang Báo cáo sản xuất</b>, không thực hiện Remove.</Rule>
     ...
     <Rule title="Read-only Alert page">Tab Production Change Alerts không tự sửa Batch/Schedule/Production; nó đọc cả ADD_JOB và REMOVE_JOB để các bên hiểu thay đổi. Từ V511, thao tác Remove downstream thực hiện tại <b>Báo cáo sản xuất</b> bởi Shift Supervisor.</Rule>
   ...
   <Section id="security-rbac" title="18 · Đăng nhập, phân quyền Role + Permission + Scope (V478 · Aiven Auth)"
    ...
    <StepList items={[
     ...
     <>Shift Supervisor có <b>production.add_job</b> và <b>Production Area scope</b> để xử lý <b>Accept & Remove Job</b> ngay trong Báo cáo sản xuất. <code>schedule.view</code> chỉ dùng để đọc cảnh báo/trạng thái trên Điều độ; <code>schedule.edit</code> không còn là quyền thay thế để thực hiện quyết định Remove downstream này.</>,
    ...
    <div className="table-wrap"><table className="erp-table"><thead><tr><th>Role mặc định</th><th>Quyền chính</th><th>Scope</th></tr></thead><tbody>
     ...
     <tr><td><b>SHIFT_SUPERVISOR</b></td><td>Operator + Add Job ngoài lô / nhận Attention + Accept & Remove upstream impact tại Báo cáo sản xuất; Điều độ chỉ xem trạng thái</td><td>Production Area</td></tr>
    </tbody></table></div>
   ...
   <section className="erp-table-panel guide-section">
    <div className="erp-panel-head"><div><b>New User Training · V496 · Live Database</b><small className="planning-sub">Training và Logic & Hướng dẫn được cập nhật song song. V496 đồng bộ cách đọc Dashboard/Planning workload và thêm Previous Main Recipe breakdown cho READY trên Scheduling; canonical Planning/Scheduling/Production engine không đổi.</small></div></div>
