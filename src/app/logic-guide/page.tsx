// V509 Global Realtime No-Supabase fail-safe leader: one visible tab polls PostgreSQL; sibling tabs use BroadcastChannel; realtime failure never blocks the app; business logic unchanged.
...
<Rule title="Hiệu năng tải trang · V437">Resolver không quét/rebuild toàn bộ <code>md_routing_detailed</code> khi mở tab. Hệ thống lọc trước Batch/Job thuộc đúng view/ngày đang xem, lấy danh sách Part/Revision liên quan, rồi mới dựng Routing Main + Masking/Unmasking cho tập nhỏ đó. Đây là tối ưu query/index, không đổi boundary Previous Main → Current Main hay logic support.</Rule>
   ...
   <Section id="database-provider" title="15 · Database Provider · Aiven PostgreSQL (V438)"
    ...
    <Rule title="Global Realtime No-Supabase · V509 fail-safe leader" tone="important">Chạy migration <code>086_global_realtime_change_event.sql</code>. Chỉ leader tab poll PostgreSQL; tab khác nhận local broadcast. Initial subscription không refresh lại RSC. Chỉ event thật và liên quan màn hình hiện tại mới soft reconcile. Realtime lỗi phải fail-open: app vẫn dùng được, không F5, không reload document, và polling tự backoff/retry.</Rule>
