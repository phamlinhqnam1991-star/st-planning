...
export async function POST(req:NextRequest){
 try{
   const body=await req.json(); const area_code=String(body.area_code||"").trim().toUpperCase(); const area_name=String(body.area_name||"").trim();
...
export async function PUT(req:NextRequest){
 try{
   const body=await req.json(); const id=Number(body.id); if(!id)return NextResponse.json({error:"Area ID không hợp lệ."},{status:400});
