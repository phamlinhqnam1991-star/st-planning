...
export async function GET(req:NextRequest){
 ...
 const nextMain=String(url.searchParams.get("nextMain")||"").trim()||null;
 const nextRecipeKey=String(url.searchParams.get("nextRecipeKey")||"").trim()||null;
 ...
 try{
  const data=await loadStWorkloadQuickView(c,{standardOperation,recipeKey,status,previousMain,nextMain,nextRecipeKey});
