  ...
  const [flowQ,rawQ,mainQ,groupQ,areaQ,scheduleQ]=await Promise.all([
   c.query(`
    ...
    select
      ...
      true st_scope,cat.operation_type,
   ...
   c.query(`
    ...
    select c.operation_code,coalesce(o.operation_name,c.operation_code) operation_name,
           ...
           (
            exists(select 1 from md_st_operation_scope scope where scope.is_active=true and operation_type<>'INTERMEDIATE' and upper(trim(scope.operation_code))=c.operation_code)
            or exists(
              ...
              where upper(trim(bo.operation_code))=c.operation_code
            )
           ) in_st_scope
    from catalog c
    left join lateral (
