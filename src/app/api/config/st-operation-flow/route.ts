...
const OPERATION_TYPES=new Set(["PLANNING_OPERATION","ST_SCOPE_ONLY"]);
 ...
 try{
  const [flowQ,rawQ,mainQ,groupQ,areaQ,scheduleQ]=await Promise.all([
   c.query(`
    ...
    select
      scope.operation_code,coalesce(o.operation_name,scope.operation_code) operation_name,o.planning_sort_order,
      true st_scope,scope.operation_type,
    ...
    left join lateral (
      ...
      where upper(trim(m.source_operation_code))=scope.operation_code
    ...
    left join lateral (
      ...
      where j.is_open=true
        and upper(trim(j.next_operation))=scope.operation_code
    ) j on true
   ...
   c.query(`
    ...
    select c.operation_code,coalesce(o.operation_name,c.operation_code) operation_name,
           ...
           exists(
            ...
            where scope.is_active=true and upper(trim(scope.operation_code))=c.operation_code
           ) in_st_scope
    from catalog c
    left join lateral (
