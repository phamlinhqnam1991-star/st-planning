...
export const ST_AI_LOGIC_SECTIONS:StLogicSection[]=[
 ...
 },
 {
  ...
  content:`V511 moves the operator-facing Shift Accept & Remove action for V506 REMOVE BEFORE START impacts from Scheduling Board to Production Execution. Scheduling Board still shows the upstream REMOVE impact, affected Batch, WAITING SHIFT ACCEPT / ACCEPTED / CRITICAL status and a link to the correct Production Report date, but it no longer performs the removal. Production Execution loads REMOVE_JOB handover events for the visible scheduled Batch population and renders Upstream Impact · Shift Action directly under each affected Batch. A Shift Supervisor with production.add_job and matching Production Area scope may Accept & Remove Job there; the downstream Batch membership is then recomputed, totals/process time are refreshed by the existing acceptDownstreamRemove transaction, Internal Chat notification remains automatic, and Scheduling/Production reconcile through Global Realtime No-Supabase. A NEW REMOVE impact still blocks first Start. If the affected Batch already started, the Production Report shows CRITICAL · BATCH ALREADY STARTED and automatic removal stays blocked. The accept API now requires production.add_job + Production Area scope; schedule.edit alone no longer authorizes this production decision. No READY/WAIT, Recipe, Batch compatibility, Schedule dependency or Remove Before Start source logic changes.`
 },
 {
