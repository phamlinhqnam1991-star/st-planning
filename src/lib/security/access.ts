...
export type AccessQueryClient={query:(sql:string,params?:unknown[])=>Promise<{rowCount:number|null;rows:any[]}>};
...
/** V516: allows API routes that already own a PostgreSQL client to reuse it for
 * authorization instead of checking out a second client from DB_POOL_MAX=1. */
export async function getAccessContextWithClient(c:AccessQueryClient):Promise<AccessContext|null>{
 const token=await readSessionToken();if(!token)return null;
 ...
 // Keep permission side-reads sequential on this one client. This is deliberate
 // for Aiven/Vercel where DB_POOL_MAX commonly stays at 1.
 const rolesQ=await c.query(`select role_key from app_user_role where user_id=$1 order by role_key`,[p.user_id]);
 const permsQ=await c.query(`select distinct rp.permission_key from app_user_role ur join app_role_permission rp on rp.role_key=ur.role_key where ur.user_id=$1 union select permission_key from app_user_permission where user_id=$1 and allowed=true except select permission_key from app_user_permission where user_id=$1 and allowed=false`,[p.user_id]);
 const scopesQ=await c.query(`select scope_type,scope_key from app_user_scope where user_id=$1 order by scope_type,scope_key`,[p.user_id]);
...
export async function getAccessContext():Promise<AccessContext|null>{
 // Preserve the historical fast path: no session cookie means no DB checkout.
 const token=await readSessionToken();if(!token)return null;
 const c=await getPool().connect();
 try{return await getAccessContextWithClient(c as AccessQueryClient);}
 finally{c.release();}
}
