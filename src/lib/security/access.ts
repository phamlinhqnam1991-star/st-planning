...
export function firstAllowedPath(ctx:AccessContext){const order:Array<[PermissionKey,string]>=[["dashboard.view","/dashboard"],["jobs.view","/all-open-jobs"],["planning.view","/planning"],["schedule.view","/schedule"],["production.view","/production-execution"],["tracking.view","/job-tracker"],["master.view","/master-data"],["config.view","/settings"],["guide.view","/logic-guide"],["training.view","/training"],["security.manage","/users-permissions"]];return order.find(([p])=>ctx.permissions.has(p))?.[1]||"/access-denied";}
